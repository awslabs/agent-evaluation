...
def sagemaker_endpoint_fixture(mocker):
    ...
    fixture = target.SageMakerEndpointTarget(
...
class TestSageMakerEndpointTarget:
    def test_base_args(self, sagemaker_endpoint_fixture):
        assert sagemaker_endpoint_fixture._args == {
            ...
            "ContentType": target._CONTENT_TYPE,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "test prompt 2",
                {"inputs": {"prompt": None}},
                target.parse("$.inputs.prompt"),
                {"inputs": {"prompt": "test prompt 2"}},
            ),
            (
                "test prompt 3",
                [{"inputs": None}],
                target.parse("$.[0].inputs"),
                [{"inputs": "test prompt 3"}],
            ),
            (
                "test prompt 4",
                {"inputs": [{"prompt": None}]},
                target.parse("$.inputs.[0].prompt"),
    ...
    def test_update_request(
        ...
        assert sagemaker_endpoint_fixture._args["Body"] == target.json.dumps(expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                target.parse("$.[0].generated_text"),
                "test completion 1",
            ),
            (
                {"output": {"completion": "test completion 2"}},
                target.parse("$.output.completion"),
                "test completion 2",
            ),
            (
                {"output": [{"completion": "test completion 3"}]},
                target.parse("$.output.[0].completion"),
                "test completion 3",
            ),
        ],
    )
